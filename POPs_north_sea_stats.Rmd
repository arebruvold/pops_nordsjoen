...
* Explanatory variables:
  ...
  + fish age (years, omitted)

From previous studies we know the effect of these variables will differ between species due to e.g. different physiology and feeding habits. Hence, including fish as a factor in a single model will require an advanced model and was not justified since comparisons between the levels of the different fish species were not important. Known confounders are fish age or proxies such as fish length and fish weight as well as liver weight, liver fat content, hepatosomatic index and liver somatic index.
...
However, it appears to be a decrease in LSI for haddock with year due to fish weight/size increase.
...
ggsave("conditional_pr_stepan_data.svg", width = 8, height = 6)
...
# pmtest <- p_man %>%
#   unnest(data) %>%
#   select(value, lsi_pc, fat_pc, year, length_cm) %>%
#   ungroup() %>%
#   mutate(
#     value = 10^value - 1
#   )
