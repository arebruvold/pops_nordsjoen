...
predictors <- c("length_cm", "fish_weight_g", "liver_weight_g", "fat_pc", "lsi_pc", "year", "species", "sample_name", "alder")
...
p_man %>%
  ...
  xlab("Year") +
  #change theme to make the minor and major tick lines in the background the same size
...
p_man %>%
  ...
  geom_jitter(size = 0.3, na.rm = T, height = 0, width = 0.25) +
  stat_smooth(method = "lm", na.rm = TRUE, se = FALSE, color = "red", size = 0.5) +
  # fit a linear curve, with color gradient dependent on the value of the p-value
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    aes(color = 10^mean(year_estimate, na.rm = TRUE) - 1, fill = 10^mean(year_estimate, na.rm = TRUE) - 1),
    size = 5,
    se = FALSE
  ) +
  # conditional coloring percentage change
