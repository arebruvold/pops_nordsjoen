...
pop_lever <- read_csv2("data/Hele datasettet lipidvekt til Sylvia med fys param sfr.csv") %>%
  clean_names() %>%
  select(year, area, species, hel_lengde_cm, fettinnhold_percent, matches("log.+ww")) %>%
  filter(if_any(matches("log"), ~ !is.na(.x))) %>%
  mutate(hel_lengde_cm = log10(hel_lengde_cm)) %>% 
...
```
## Added-variable or partial regression plot TODO
