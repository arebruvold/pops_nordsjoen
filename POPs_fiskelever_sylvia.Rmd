...
full_raw_data <- read_excel("data/20230424_nordsjoen_stepan.xlsx", skip = 1) %>%
    clean_names() %>% 
    select(species, year, hel_vekt_g, length_cm, fettinnhold_percent, matches("w_w")) %>%
    drop_na(species, year, length_cm, fettinnhold_percent) %>%
    mutate(across(.cols = matches("w_w"), .fn = ~ log10(as.numeric(.x) + 1))) %>%
      mutate(across(.cols = matches("length_cm"), .fn = ~ log10(as.numeric(.x)))) %>%
  mutate(across(.cols = matches("fettinnhold_percent|year|hel_vekt_g"), .fn = ~ as.numeric(.x))) %>% 
    rename_with(
      ~ str_replace_all(.x, c(
        "w_w" = "ww",
        "^s" = "",
...
selection_man <- full_raw_data %>%
  ...
  drop_na(value) %>% 
  nest() %>% 
  # apply m1_y_l_f_w and calculate medians of covariates
  mutate(
    m1_y_l_f_w = map(data, lm_y_l_f_w),
    m2_y_l_f = map(data, lm_y_l_f),
    m3_y_l = map(data, lm_y_l),
    m3_y = map(data, lm_y),
    med_length_cm = map(
      data,
      ~ median(.x$length_cm, na.rm = TRUE)
  ...
  mutate(across(matches("m\\d"), ~ map(.x, AIC), .names = "{.col}_AIC")) %>% 
  mutate(
    glance = map(m1_y_l_f_w, broom::glance),
    year_pval = map(m1_y_l_f_w, ~ broom::tidy(.x)[[2, 5]] %>% as.numeric()),
    # tidy gets summary variables of the m1_y_l_f_w coefficients, pivot to get coeffs for unnest
    tidy = map(m1_y_l_f_w, ~ broom::tidy(.x) %>%
      select(term, estimate) %>%
      pivot_wider(names_from = "term", values_from = "estimate") %>%
      clean_names() %>%
      rename_with(.cols = everything(), .fn = ~ paste0(.x, "_coeff"))),
    # augment gets per observation variables (e.g., residuals)
    augment = map2(m1_y_l_f_w, data, ~ broom::augment(.x, .y)),
    partial_resids = map(m1_y_l_f_w, ~ residuals(.x, type = "partial") %>%
      as_tibble() %>%
      rename_with(.cols = everything(), .fn = ~ paste0(.x, "_partial_resid"))),
    normal_resids = map(m1_y_l_f_w, ~ residuals(.x) %>%
      as_tibble() %>%
      rename_with(.cols = everything(), .fn = ~ paste0(.x, "_normal_resid")))
  ) %>%
  # add package-generated plots for an extra check
...
collinearity only a problem for the collinear variables themselves.

Apply model 1, 2, 3 and print tables for each showing AIC.
